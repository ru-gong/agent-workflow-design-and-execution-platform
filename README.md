...
默认启动会在用户输入需求后调用当前选择的真实编程工具（Codex 或 Claude Code）生成编排；如果调用失败，页面会直接提示错误，不会自动退回到本地测试草案。

如果只想体验界面和编排，不想消耗真实工具调用，需要显式开启 mock 模式：
...
- Token 预算估算升级：执行前预览不再按编排 JSON 大小粗略估算，而是综合节点提示词模板、全局需求、上游上下文、预期输出、运行日志和工具调用开销，展示保守区间与提示词/输出/日志拆分。
- 前后端估算口径统一：运行时 token 累计使用同一套中英文混合文本估算器，中文任务不再被明显低估。
